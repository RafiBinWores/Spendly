            ...
            <div>
                ...
                <div x-data="{
                    ...
                    <input multiple x-ref="files" type="file"
                        ...
                        wire:model.live="newFiles" {{-- <— IMPORTANT --}} wire:key="file-input-{{ $expenseId ?? 'new' }}"
                        x-on:change="$nextTick(()=>{$refs.files.value=null})" />

                    {{-- New uploads (show filename) --}}
                            ...
                            <div class="relative h-[100px] w-full rounded-xl border overflow-hidden group bg-slate-100">
                                ...
                                @else
                                    <div class="flex flex-col items-center justify-center w-full h-full text-xs p-2">
                                        <span
                                            class="inline-grid place-items-center w-10 h-10 rounded-lg bg-slate-200 text-slate-700 font-semibold uppercase">
                                            {{ $ext }}
                                        </span>
                            ...
                            <div class="relative h-[100px] w-full rounded-xl border overflow-hidden group bg-slate-100">
                                @if ($isImg)
                                    <a href="{{ asset('storage/' . $p) }}" target="_blank">
                                        <img src="{{ asset('storage/' . $p) }}" class="object-contain w-full h-full"
                                            alt="{{ $name }}">
                                    </a>
                                @else
                                    <a href="{{ asset('storage/' . $p) }}" target="_blank"
