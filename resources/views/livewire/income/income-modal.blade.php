        ...
        <form wire:submit="submit" class="space-y-6">
            ...
            <div class="form-group" x-data="{
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="relative w-10 h-10 flex items-center justify-center rounded-lg border border-neutral-200 bg-white
                    dark:bg-neutral-800 dark:border-neutral-700">
                        ...
                        <div wire:loading.delay wire:target="icon,iconStyle"
                    ...
                    <div class="flex items-center gap-2">
                       ...
                       focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-500 dark:focus:ring-offset-neutral-900"
                            @click="open = !open" :disabled="@js($isView)" wire:loading.attr="disabled"
                        ...
                        @if ($icon && !$isView)
                           ...
                           dark:bg-neutral-800 dark:text-neutral-200 dark:border-neutral-700 dark:hover:border-neutral-600
                           focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-500 dark:focus:ring-offset-neutral-900"
                                wire:click="$set('icon','')" wire:loading.attr="disabled" wire:target="icon">
                                <span>Clear</span>
                                <svg wire:loading wire:target="icon" class="animate-spin w-4 h-4" viewBox="0 0 24 24"
                                    fill="none">
                        ...
                        <div
                            class="inline-flex rounded-lg border border-neutral-200 dark:border-neutral-700 overflow-hidden ml-2">
                            <button type="button"
                                class="relative px-3 py-1 text-sm bg-white dark:bg-neutral-800 dark:text-neutral-200 cursor-pointer"
                                :class="@js($iconStyle) === 'o' ?
                                    'bg-neutral-100 dark:bg-neutral-900 font-medium' : ''"
                                wire:click="$set('iconStyle','o')" @disabled($isView)
                                wire:loading.attr="disabled" wire:target="iconStyle">
                                Outline
                                <svg wire:loading wire:target="iconStyle"
                                    class="animate-spin w-4 h-4 inline-block ml-1 align-[-2px]" viewBox="0 0 24 24"
                                    fill="none">
                                    <circle class="opacity-25" cx="12" cy="12" r="10"
                                        stroke="currentColor" stroke-width="3" />
                                    <path class="opacity-90" fill="currentColor"
                            ...
                            <button type="button"
                                class="relative px-3 py-1 text-sm bg-white dark:bg-neutral-800 dark:text-neutral-200 cursor-pointer"
                                :class="@js($iconStyle) === 's' ?
                                    'bg-neutral-100 dark:bg-neutral-900 font-medium' : ''"
                                wire:click="$set('iconStyle','s')" @disabled($isView)
                                wire:loading.attr="disabled" wire:target="iconStyle">
                                Solid
                                <svg wire:loading wire:target="iconStyle"
                                    class="animate-spin w-4 h-4 inline-block ml-1 align-[-2px]" viewBox="0 0 24 24"
                ...
                <div x-cloak x-show="open" @click.outside="open = false" x-transition class="relative z-20 mt-3">
                    <div
                        class="absolute left-0 right-0 rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-lg p-3
                        ...
                        scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700">
                            @foreach ($availableIcons as $name)
                               ...
                               dark:bg-neutral-800 dark:border-neutral-700 dark:hover:border-neutral-600"
                                    wire:click="$set('icon','{{ $name }}')" wire:loading.attr="disabled"
                                    wire:target="icon" @disabled($isView)>
                                    <x-dynamic-component :component="'heroicon-' . $iconStyle . '-' . $name"
                                        class="w-6 h-6 text-neutral-800 dark:text-neutral-200" />
                                    <span
                                        class="text-[10px] text-neutral-600 dark:text-neutral-300 truncate w-full text-center">
                                        {{ $name }}
                                    </span>
            ...
            {{-- Amount --}}
            <div class="form-group">
                <flux:input type="number" min="0" :disabled="$isView"
                    wire:model="amount" label="Amount" placeholder="e.g. 20,000" />
            </div>
